...
AST_N_MELS = 128
AST_INPUT_TDIM = 1024
AST_MAX_SAMPLES = (AST_INPUT_TDIM - 1) * AST_HOP_LENGTH + AST_N_FFT
AST_EMBED_DIM = 768


class ASTWrapper(nn.Module):
    def __init__(self, model_fp, device, ast_input_tdim=AST_INPUT_TDIM, ast_label_dim=527,
                 use_ast_default_pretrain_if_path_invalid=False):
        super().__init__()
        self.device = device
        self.ast_input_tdim = ast_input_tdim
        self.ast_label_dim = ast_label_dim

        self.ast_model = ASTModel(
            label_dim=self.ast_label_dim,
            input_tdim=self.ast_input_tdim,
            audioset_pretrain=False
        ).to(device)

        loaded_successfully = False
        if model_fp and os.path.exists(model_fp):
            checkpoint = torch.load(model_fp, map_location=device)
            actual_model_state_dict = checkpoint
            if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
                actual_model_state_dict = checkpoint['state_dict']
            elif isinstance(checkpoint, dict) and 'model' in checkpoint:
                actual_model_state_dict = checkpoint['model']

            if actual_model_state_dict and 'module.' in list(actual_model_state_dict.keys())[0]:
                actual_model_state_dict = {k.replace('module.', ''): v for k, v in actual_model_state_dict.items()}

            model_dict = self.ast_model.state_dict()
            pretrained_dict = {k: v for k, v in actual_model_state_dict.items() if
                               k in model_dict and model_dict[k].shape == v.shape}
            missing_keys, unexpected_keys = self.ast_model.load_state_dict(pretrained_dict, strict=False)

            if not missing_keys and not unexpected_keys:
                loaded_successfully = True
                print(f"Successfully loaded AST model from {model_fp}.")
            elif pretrained_dict:
                loaded_successfully = True
                print(f"Partially loaded AST model from {model_fp}.")
                if missing_keys: print(f"Missing keys in AST model: {missing_keys}")
                if unexpected_keys: print(f"Unexpected keys in checkpoint: {unexpected_keys}")
            else:
                print(f"Warning: No matching keys found loading AST checkpoint at {model_fp}.")
        elif model_fp:
            print(f"Warning: AST pretrained_path '{model_fp}' not found.")

        if not loaded_successfully and use_ast_default_pretrain_if_path_invalid:
            print(f"Attempting AST default Audioset pretraining.")
            try:
                self.ast_model = ASTModel(
                    label_dim=self.ast_label_dim,
                    input_tdim=self.ast_input_tdim,
                ...
                sample_weight_after_fallback = next(self.ast_model.parameters()).sum()
                temp_model_random = ASTModel(label_dim=self.ast_label_dim, input_tdim=self.ast_input_tdim,
                                             audioset_pretrain=False)
                sample_weight_random = next(temp_model_random.parameters()).sum()
                if not torch.isclose(sample_weight_after_fallback, sample_weight_random):
                    print("AST model re-initialized and appears to have loaded weights with audioset_pretrain=True.")
                    loaded_successfully = True
                else:
                    print("AST model re-initialized with audioset_pretrain=True, but weights seem unchanged.")
            except Exception as e:
                print(f"Error during AST model fallback initialization: {e}")

        if not loaded_successfully and not model_fp and not use_ast_default_pretrain_if_path_invalid:
            print("AST model initialized with random weights (no path, no fallback).")
        elif not loaded_successfully:
            print("AST model may have random weights (loading/fallback issues).")

    def _load_and_process_audio(self, wav_path):
        waveform, sr = librosa.load(wav_path, sr=AST_SAMPLE_RATE, mono=True)
        if len(waveform) > AST_MAX_SAMPLES:
            waveform = waveform[:AST_MAX_SAMPLES]
        elif len(waveform) < AST_MAX_SAMPLES:
            padding = AST_MAX_SAMPLES - len(waveform)
            waveform = np.pad(waveform, (0, padding), mode='constant')

        mel_spectrogram = librosa.feature.melspectrogram(
            y=waveform, sr=AST_SAMPLE_RATE, n_fft=AST_N_FFT,
            hop_length=AST_HOP_LENGTH, n_mels=AST_N_MELS, center=False
        )
        if mel_spectrogram.shape[1] > self.ast_input_tdim:
            mel_spectrogram = mel_spectrogram[:, :self.ast_input_tdim]
        elif mel_spectrogram.shape[1] < self.ast_input_tdim:
            padding_width = self.ast_input_tdim - mel_spectrogram.shape[1]
            mel_spectrogram = np.pad(mel_spectrogram, ((0, 0), (0, padding_width)), mode='constant', constant_values=0)
        return torch.tensor(mel_spectrogram, dtype=torch.float32)

    def get_audio_embeddings(self, file_paths: list):
        batch_mel_spectrograms = [self._load_and_process_audio(path) for path in file_paths]
        inputs = torch.stack(batch_mel_spectrograms, dim=0).to(self.device)
        inputs = inputs.unsqueeze(1)
        return self.ast_model(inputs, return_embedding_only=True)
...
class AudioDataset_Bayesian(Dataset):
    def __init__(self, feature_dir, poi_json_file_path):
        ...
        try:
            ...
            print(f"Error: POI JSON file not found at {poi_json_file_path}. Dataset will be empty.")
        except (json.JSONDecodeError, UnicodeDecodeError) as e:
            print(f"Error reading POI JSON from {poi_json_file_path}: {e}. Dataset may be impacted.")
        if not self.all_data:
            print(f"Warning: No valid data loaded from {poi_json_file_path} for AudioDataset_Bayesian.")
...
class AudioDataset_EmbeddingPOI(Dataset):
    def __init__(self, feature_dir, poi_json_file_path, poi_feat_dir):
        ...
        try:
            ...
            print(f"Error reading POI JSON from {poi_json_file_path}: {e}. Dataset may be impacted.")
        if not self.all_data:
            print(f"Warning: No valid data loaded from {poi_json_file_path} for AudioDataset_EmbeddingPOI.")
        self.label_map = AUDIO_LABEL_TO_IDX
...
class AudioClassifier_Bayesian(nn.Module):
    def __init__(self, num_labels, model_weights_path, device, use_poi=False, prior_strength=0.5, priors_dir=None,
                 use_ast_default_pretrain_fallback=False, finetune_ast=True):
        super().__init__()
        ...
        self.ast_wrapper = ASTWrapper(model_fp=model_weights_path, device=device,
                                      use_ast_default_pretrain_if_path_invalid=use_ast_default_pretrain_fallback)
        if not finetune_ast:
            print(f"Freezing AST model parameters for {self.__class__.__name__}.")
            for param in self.ast_wrapper.ast_model.parameters():
        ...
        if self.use_poi:
            ...
            try:
                ...
                self.use_poi = False
        self.time_predictor = nn.Linear(AST_EMBED_DIM, 2).to(device)

    def forward(self, file_paths: list, batch_poi_multi_hot=None):
        audio_embeddings = self.ast_wrapper.get_audio_embeddings(file_paths)
...
class AudioClassifier_CrossAttention(nn.Module):
    def __init__(self, num_labels, model_weights_path, device, use_poi=False,
        ...
        self.audio_feature_projector = nn.Sequential(
            nn.Linear(AST_EMBED_DIM, self.projected_audio_dim),
    ...
    def forward(self, file_paths: list, batch_poi_embeddings=None):
        audio_ast_embeddings = self.ast_wrapper.get_audio_embeddings(file_paths)
        audio_ast_embeddings_float = audio_ast_embeddings.to(torch.float32)
        projected_audio_features = self.audio_feature_projector(audio_ast_embeddings_float)
        time_outputs = self.time_predictor(audio_ast_embeddings_float)
...
class AudioClassifier_FiLM(nn.Module):
    def __init__(self, num_labels, model_weights_path, device, use_poi=False,
                 poi_embed_dim=POI_BERT_EMBED_DIM, projected_audio_dim=512,
                 film_poi_hidden_dim=256, use_ast_default_pretrain_fallback=False,
                 finetune_ast=True):
        super().__init__()
        self.use_poi = use_poi
        self.device = device
...
def train_model(model, dataloader, criterion, optimizer):
    ...
    for batch_data in dataloader:
        _, file_paths, event_spans, labels, poi_data = batch_data  # audio_ids not used in train
...
def evaluate_model(model, dataloader):
    ...
    with torch.no_grad():
        for batch_data in dataloader:
            _, file_paths, event_spans, labels, poi_data = batch_data  # audio_ids not used in eval
...
def main():
    ast_model_weights_path = os.path.join("..", "audioset_10_10_0.4593.pth")
    ast_use_default_fallback = False
    ...
    output_results_dir = os.path.join("..", "outputs",
                                      "results_all_AST")
    ...
    common_model_params = {
        "model_weights_path": ast_model_weights_path,
        "use_ast_default_pretrain_fallback": ast_use_default_fallback
    }
    ...
    experiments = {
        "bayesian_audio_only": {
            ...
            "model_params": {"prior_strength": 0.5, "priors_dir": None, **common_model_params, "finetune_ast": False},
            "dataset_params": {"poi_json_file_path": poi_and_labels_json_file}
        },
        "bayesian_fusion": {
            "model_class": AudioClassifier_Bayesian, "dataset_class": AudioDataset_Bayesian,
            "collate_fn": collate_fn_bayesian, "use_poi": True,
            "model_params": {"prior_strength": 0.5, "priors_dir": poi_bayesian_priors_dir, **common_model_params,
                             "finetune_ast": True},  # Example: Finetune AST
            "dataset_params": {"poi_json_file_path": poi_and_labels_json_file}
        },
        "cross_attention_fusion": {
            ...
            "model_params": {"projected_audio_dim": 512, "poi_embed_dim": POI_BERT_EMBED_DIM, **common_model_params,
                             "finetune_ast": True},
    ...
    for exp_name, config in experiments.items():
        ...
        model_constructor_params = {
            "num_labels": NUM_AUDIO_LABELS, "device": device,
            "use_poi": config["use_poi"], **config["model_params"]
        }
        model = config["model_class"](**model_constructor_params)
        model.to(device)
